...
###########################    MAIN    ##############################
def main(SEED=None):
    print('AG-DoorINet training script')
    if SEED is not None:
    ...
    model_name = 'AG-DoorINet'
    ag_doorinet = AG_DoorINet(drop_out=0.1, model_name=model_name).to(DEVICE)

    for name, param in ag_doorinet.named_parameters():
        if param.dim()>=2:
            nn.init.xavier_uniform_(param)
    params = sum(p.numel() for p in ag_doorinet.parameters() if p.requires_grad)*1e-3
    paramstr = '{mod_name} has {parameters:.2f}K parameters\n'
    print(paramstr.format(mod_name=model_name, parameters=params))
    optimizer = torch.optim.AdamW(params=ag_doorinet.parameters(), lr=1e-3)
    ...
    history = train(x_train, x_val, y_train, y_val, model=ag_doorinet, criterion=loss_fn, epochs=150, sched=scheduler, opt=optimizer,
                    DEVICE=DEVICE, start_epoch=0)

    print('saving the model...')
    model_fname = os.path.join(MODELS_FOLDER, 'AG_DoorINet_' +datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")+'_seed_'+str(SEED)+'.pt')
    torch.save(ag_doorinet.state_dict(), model_fname)
    print('AG-DoorINet saved as', model_fname)
    print('finish')
